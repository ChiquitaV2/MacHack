	...
	public static void downloadSongs(boolean log) {
		try {
			FileUtils.copyURLToFile(
					...
					MacFileMang.getDir().resolve("notebot").resolve("songs.zip").toFile());
			ZipFile zip = new ZipFile(MacFileMang.getDir().resolve("notebot").resolve("songs.zip").toFile());
			Enumeration<? extends ZipEntry> files = zip.entries();
			int count = 0;
			while (files.hasMoreElements()) {
				count++;
				ZipEntry file = files.nextElement();
				File outFile = MacFileMang.getDir().resolve("notebot").resolve(file.getName()).toFile();
			...
			zip.close();
			Files.deleteIfExists(MacFileMang.getDir().resolve("notebot").resolve("songs.zip"));

			if (log) MacLogger.infoMessage("Downloaded " + count + " Songs");
		} catch (Exception e) { if (log) MacLogger.warningMessage("Error Downloading Songs... " + e); e.printStackTrace(); }
	}
