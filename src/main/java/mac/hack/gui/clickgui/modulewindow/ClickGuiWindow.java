...
public abstract class ClickGuiWindow extends Window {
	...
	public void fillGrey(MatrixStack matrix, int x1, int y1, int x2, int y2) {
		...
		RenderUtils.drawRect(x1M, y1M + 1, x2M - 1, y2M, ColorUtils.guiColour() + 0x010101, 0.20f);
		RenderUtils.drawRect(x1M, y1M + 11, x2M, y1M + 12, ColorUtils.guiColour(), 1f);
		DrawableHelper.fill(matrix, x1, y1 + 2, x1 + 1, y2, ColorUtils.guiColour() + 0x010101);
		DrawableHelper.fill(matrix, x1 + 1, y1 + 1, x1 + 2, y1 + 2, ColorUtils.guiColour() + 0x010101);
		DrawableHelper.fill(matrix, x2 - 2, y2 - 2, x2 - 1, y2 - 1, ColorUtils.guiColour() + 0x010101);
		DrawableHelper.fill(matrix, x1, y2, x2 - 2, y2 - 1, ColorUtils.guiColour() + 0x010101);
		DrawableHelper.fill(matrix, x1 + 2, y1, x2, y1 + 1, ColorUtils.guiColour() + 0x010101);
		DrawableHelper.fill(matrix, x2, y2 - 2, x2 - 1, y1, ColorUtils.guiColour() + 0x010101);
	}
