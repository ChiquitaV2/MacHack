...
public class ServerScraperScreen extends Screen {
	...
	private List<MacServerPinger> pingers = new ArrayList<>();
		...
		scrapeThread = new Thread(() -> {
			for (int change : new int[] {0, -1, 1, -2, 2, -3, 3}) {
				for (int i = 0; i <= 255; i++) {
					...
					MacServerPinger ping = new MacServerPinger();
	...
	public void updatePingers() {
		for (MacServerPinger ping: new ArrayList<>(pingers)) {
			if (ping.done) {
				checked++;
