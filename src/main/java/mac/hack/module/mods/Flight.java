...
public class Flight extends Module {
    ...
    public void onTick(EventTick event) {
        ...
        if (getSettings().get(0).asMode().mode == 0) {
            ...
            if (getSettings().get(2).asMode().mode == 0 || getSettings().get(2).asMode().mode == 3)
                mc.player.setVelocity(0, 0, 0);
            else if (getSettings().get(2).asMode().mode == 1 && WorldUtils.NONSOLID_BLOCKS.contains(mc.world.getBlockState(new BlockPos(mc.player.getPos().getX(), mc.player.getPos().getY() - 0.069, mc.player.getPos().getZ())).getBlock()))
                mc.player.setVelocity(0, mc.player.age % 20 == 0 ? -0.069 : 0, 0);
            else if (getSettings().get(2).asMode().mode == 2)
