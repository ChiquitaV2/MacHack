    ...
    public void onDrawOverlay(EventDrawOverlay event) {
        ...
        if (getSetting(0).asToggle().state && !mc.options.debugEnabled || getSetting(1).asToggle().state && !mc.options.debugEnabled) {
            ...
            if (getSetting(0).asToggle().state) {
                for (Module m : ModuleManager.getModules())
                    if (m.isToggled() && m.isDrawn() && !m.getName().equals("HUD") && !m.isHidden())
                        lines.add(m.getName());
        ...
        if (getSetting(20).asToggle().state && !mc.options.debugEnabled) {
            ...
            if (mc.player.getMainHandStack().isDamageable()) {
                dura = "Durability" + (getSetting(24).asToggle().state ? " \u00a7f" : "\u00a77: \u00a7r") + (mc.player.getMainHandStack().getMaxDamage() - mc.player.getMainHandStack().getDamage());
            } else {
