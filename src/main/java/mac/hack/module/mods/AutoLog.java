...
public class AutoLog extends Module {
    ...
    public void onLivingLabelRender(EventEntityRender.Label event) {
        if (event.getEntity() instanceof PlayerEntity && getSetting(6).asToggle().state && getSetting(8).asMode().mode == 1) {
            if (!event.getEntity().getName().equals(this.mc.player.getName()) && !MacHack.friendMang.has(event.getEntity().getDisplayName().asString()))
            {
            ...
            } else
            {
                ...
                if (this.getSettings().get(6).asToggle().state)
                {
                    ...
                    while (entityIter.hasNext())
                    {
                        ...
                        if (!e.getName().equals(this.mc.player.getName()) && (double) this.mc.player.distanceTo(e) <= this.getSettings().get(7).asSlider().getValue() && !MacHack.friendMang.has(e.getDisplayName().asString()))
                        {
