...
public class AutoWither extends Module {
    ...
    public void onEnable() {
        ...
        if (getSetting(1).asToggle().state) {
            MacLogger.warningMessage("Setting ticks passed to 0");
        }
    }
    ...
    public void onTick(EventTick event) {
        ...
        if (soulSandSlot == -1) {
            MacLogger.warningMessage("No soul sand found in your hotbar!");
            ModuleManager.getModule(AutoWither.class).toggle();
            return;
        }
        if (skullSlot == -1) {
            MacLogger.warningMessage("No wither skulls found in your hotbar!");
            ModuleManager.getModule(AutoWither.class).toggle();
            return;
        }
        ...
        if (ticksPassed == 2) {
            if(
                ...
                if (getSetting(1).asToggle().state) {
                    MacLogger.warningMessage("Attempting to place wither skulls");
                }
