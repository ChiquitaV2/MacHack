...
public class AutoArmor extends Module {
    ...
    public AutoArmor() {
        super("AutoArmor", KEY_UNBOUND, Category.PLAYER, "Automatically equips armor",
                ...
                new SettingToggle("Delay", true).withDesc("Adds a delay between equipping armor pieces to make it bypass more anticheats"),
                new SettingSlider("Delay", 0, 20, 1, 0).withDesc("How many ticks between putting on armor pieces"));
    }

    @Subscribe
    public void onTick(EventTick event) {
        if (!MacQueue.isEmpty("autoarmor_equip"))
            return;
        ...
        tickDelay = (getSettings().get(2).asToggle().state ? (int) getSettings().get(4).asSlider().getValue() : 0);
        ...
        if (is.getItem() instanceof ArmorItem || is.getItem() == Items.ELYTRA) {
            ...
            if (is.getItem() instanceof ElytraItem) {
                ...
                if (getSettings().get(1).asToggle().state)
                    ...
                    prot = 1;
            } else if (is.getMaxDamage() - is.getDamage() < 7 && getSettings().get(0).asToggle().state) {
                return 0;
            }
